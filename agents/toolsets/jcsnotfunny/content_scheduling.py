...
from .error_handling import SchedulingConflictError, SchedulingError, SchedulingValidationError
...
class ContentSchedulingTool(BaseTool):
    ...
    def __init__(self, config: dict[str, Any] | None = None, **kwargs):
        ...
        self.supported_platforms = config.get(
            "supported_platforms",
        ...
        self.logger.info(
            f"ContentSchedulingTool initialized with platforms: {self.supported_platforms}"
        )
    ...
    def execute(self, operation: str, **kwargs) -> dict[str, Any]:
        ...
        try:
            self._log_operation("content_scheduling", {"operation": operation, "params": kwargs})
            ...
            raise SchedulingError(
                f"Content scheduling failed: {e!s}",
                context={"operation": operation},
    ...
    def _generate_calendar(self, **kwargs) -> dict[str, Any]:
        ...
        while current_date <= end_date:
            ...
            if platforms_filter:
                posts = [
                    p for p in posts if any(plat in p["platforms"] for plat in platforms_filter)
                ]
    ...
    def _check_conflicts(self, **kwargs) -> dict[str, Any]:
        ...
        return {
            ...
            "conflicts": conflicts,
            "alternative_times": self._suggest_alternative_times(schedule_time, platforms)
            if conflicts
            else [],
        }
    ...
    def _optimize_schedule(self, **kwargs) -> dict[str, Any]:
        ...
        for i, post in enumerate(current_posts):
            if i < len(optimal_times):
                ...
                if (
                    abs((suggested_time - current_time).total_seconds()) > 3600
        ...
        if not found:
            raise SchedulingError(
                ...
                recovery_suggestions=[
                    ...
                    "Post may have already been canceled",
                ],
            )
    ...
    def _find_conflicts(
        self, schedule_time: datetime, platforms: list[str]
    ) -> list[dict[str, Any]]:
    ...
    def _get_posting_recommendations(
        self, schedule_time: datetime, platforms: list[str]
    ) -> dict[str, Any]:
