...
class VideoAnalysisTool(BaseTool):
    ...
    def _validate_tool_config(self) -> None:
        ...
        try:
            subprocess.run(
                [self.ffmpeg_path, "-version"], capture_output=True, check=True, timeout=5
            )
            subprocess.run(
                [self.ffprobe_path, "-version"], capture_output=True, check=True, timeout=5
            )
    ...
    def execute(self, video_path: str, analysis_type: str = "full", **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            self._log_operation(
                "video_analysis", {"video_path": video_path, "analysis_type": analysis_type}
            )
            ...
            if analysis_type == "quick":
                ...
                raise VideoAnalysisError(
                    ...
                    recovery_suggestions=["Use one of: 'quick', 'full', 'speaker', 'cuts'"],
                )
    ...
    def _generate_recommendations(self, video_info: dict[str, Any]) -> list[str]:
        ...
        if video:
            ...
            if width < 1920 or height < 1080:
                recommendations.append(
                    "Consider using higher resolution (1080p or better) for better quality"
                )
        ...
        if audio:
            ...
            if sample_rate < 44100:
                recommendations.append(
                    "Audio sample rate is low - consider using 44.1kHz or higher"
                )
