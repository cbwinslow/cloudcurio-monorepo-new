...
class Orchestrator:
    def __init__(self, orchestrator_id: str = "orchestrator_1"):
        ...
        self.channel.queue_bind(
            exchange=RESULT_EXCHANGE, queue=self.results_queue, routing_key=self.orchestrator_id
        )
        ...
        ] = {}  # vote_topic -> task_id -> {"voted_agents": [], "votes": []}
    ...
    def coordinate_consensus(self, vote_topic: str, task_id: str) -> dict[str, Any]:
        ...
        if vote_topic not in self.active_votes or task_id not in self.active_votes[vote_topic]:
            return {
                "status": "no_votes",
                "message": f"No votes found for topic '{vote_topic}' and task '{task_id}'.",
            }
        ...
        if not votes_data:
            return {
                "status": "no_votes",
                "message": f"No votes recorded for topic '{vote_topic}' and task '{task_id}'.",
            }
        ...
        if len(winning_votes) == 1:
            ...
            consensus_result["message"] = (
                f"Consensus reached: Majority voted for '{winning_votes[0]}'."
            )
            consensus_result["status"] = "success"
        else:
            consensus_result["consensus"] = winning_votes  # Multiple winners, no clear majority
            consensus_result["message"] = (
                f"No clear majority. Multiple top votes: {', '.join(winning_votes)}."
            )
            consensus_result["status"] = "tie"

        print(
            f"Consensus for topic '{vote_topic}' and task '{task_id}': {consensus_result['message']}"
        )
    ...
    def _process_vote_message(self, ch, method, properties, body):
        ...
        print(
            f" [x] Received vote '{vote}' on topic '{vote_topic}' for task {task_id} from agent {message.sender_id}"
        )
        ...
        if message.sender_id not in self.active_votes[vote_topic][task_id]["voted_agents"]:
            ...
            print(
                f"Warning: Agent {message.sender_id} already voted for task {task_id} on topic {vote_topic}."
            )
    ...
    def start_listening(self):
        ...
        self.channel.basic_consume(
            queue=self.results_queue, on_message_callback=self.collect_results, auto_ack=False
        )
        self.channel.basic_consume(
            queue=self.vote_queue, on_message_callback=self._process_vote_message, auto_ack=False
        )
...
"""
    ...
    try:
        ...
        security_task_id = orchestrator.assign_task(
            security_agent_id, {"code_diff": sample_code_diff}, task_type="review_code"
        )
        performance_task_id = orchestrator.assign_task(
            performance_agent_id, {"code_diff": sample_code_diff}, task_type="review_code"
        )
        ...
        print("\n--- Task Results ---")
        print(
            f"Security Task {security_task_id} status: {orchestrator.tasks.get(security_task_id)}"
        )
        print(
            f"Performance Task {performance_task_id} status: {orchestrator.tasks.get(performance_task_id)}"
        )
        ...
        orchestrator.initiate_vote(
            security_task_id, vote_topic, options
        )  # Using security task_id for context
        ...
        performance_agent.send_vote(
            performance_agent_ch, performance_task_id, "Needs More Info", vote_topic
        )
