...
class TestingAgent(BaseAgent):
    ...
    def handle_task(self, task_type: str, details: dict[str, Any]) -> dict[str, Any]:
        print(
            f"TestingAgent {self.agent_id} handling task type: {task_type} with details: {details}"
        )
        ...
        if task_type == "review_code":
            ...
            try:
                review_response = self._generate_llm_response(
                    prompt_template_name="review_testing.txt", placeholders={"CODE_DIFF": code_diff}
                )
