...
class PerformanceAgent(BaseAgent):
    ...
    def handle_task(self, task_type: str, details: dict[str, Any]) -> dict[str, Any]:
        print(
            f"PerformanceAgent {self.agent_id} handling task type: {task_type} with details: {details}"
        )
        ...
        if task_type == "review_code":
            ...
            if not code_diff:
                return {
                    "status": "error",
                    "message": "No code diff provided for performance review.",
                }
            ...
            try:
                ...
                return {
                    "status": "error",
                    "message": f"Error during Gemini performance review: {e}",
                }

        return {
            "status": "error",
            "message": f"Unknown task type: {task_type} for PerformanceAgent.",
        }
