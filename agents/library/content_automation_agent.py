...
class ContentAutomationAgent(BaseAgent):
    ...
    def _load_platform_configs(self) -> dict[str, Any]:
        ...
        return {
            ...
            "twitter": {"max_tweet_length": 280, "optimal_hashtags": 2, "max_media": 4},
            "facebook": {"max_post_length": 63206, "optimal_hashtags": 5, "max_duration": 240},
        }
    ...
    def add_to_queue(self, content: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error adding content to queue: {e!s}")
            return False
                ...
                try:
                    ...
                    if mcp_result.get("success", False):
                        ...
                        result["errors"].append(
                            f"{platform}: {mcp_result.get('error', 'Unknown error')}"
                        )

                except Exception as e:
                    result["platforms"][platform] = {"status": "failed", "error": str(e)}
                    result["errors"].append(f"{platform}: {e!s}")

            result["success"] = len(result["errors"]) == 0
            return result

        except Exception as e:
            result["errors"].append(f"Distribution failed: {e!s}")
            return result
    ...
    def _adapt_for_youtube(self, content: dict[str, Any], platform: str) -> dict[str, Any]:
        ...
        if "description" in content:
            content["description"] = self._optimize_youtube_description(
        ...
        if platform == "youtube" and "chapters" in content:
            content["description"] += "\n\n" + self._generate_youtube_timestamps(
                content["chapters"]
            )
    ...
    def _generate_utm_params(self, content: dict[str, Any], platform: str) -> dict[str, str]:
        ...
        return {
            "utm_source": platform,
            "utm_medium": "social"
            if platform in ["twitter", "facebook", "instagram", "linkedin", "tiktok"]
            else "organic",
    ...
    def _generate_youtube_tags(self, content: dict[str, Any]) -> list[str]:
        ...
        current_length = (
            sum(len(tag) for tag in unique_tags) + len(unique_tags) - 1
        )  # +1 for commas
        ...
        try:
            ...
            payload = {
                ...
                "api_key": self.api_keys.get(
                    f"mcp_{'video' if 'youtube' in platform else 'social'}", ""
                ),
            }
            ...
            return {"success": False, "error": f"Request failed: {e!s}", "platform": platform}
        except Exception as e:
            return {"success": False, "error": f"Unexpected error: {e!s}", "platform": platform}
    ...
    def generate_youtube_clips(self, episode: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if not analysis_result.get("success", False):
                self.logger.error(
                    f"Video analysis failed: {analysis_result.get('error', 'Unknown error')}"
                )
                return clips
            ...
            if len(clips) < 3:
                top_segments = sorted(
                    analysis_result.get("segments", []), key=lambda x: x["score"], reverse=True
                )[:3]
            ...
            self.logger.error(f"Error generating YouTube clips: {e!s}")
            return []
    ...
    def _analyze_video_for_clips(self, episode: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Analysis error: {e!s}"}

    def _create_clip_from_segment(
        self, episode: dict[str, Any], segment: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _generate_clip_title(
        self, episode: dict[str, Any], segment: dict[str, Any], clip_type: str
    ) -> str:
        ...
        try:
            ...
            for platform in platforms:
                try:
                    ...
                    result["errors"].append({"platform": platform, "error": str(e)})

            result["success"] = len(result["errors"]) == 0
            return result

        except Exception as e:
            result["errors"].append(f"Scheduling failed: {e!s}")
            return result
    ...
    def track_content_performance(self, content_id: str, platform: str) -> dict[str, Any]:
        ...
        try:
            ...
            payload = {
                ...
                "api_key": self.api_keys.get(
                    f"mcp_{'social' if platform in ['twitter', 'facebook', 'instagram', 'tiktok', 'linkedin'] else 'video'}",
                    "",
                ),
            }
            ...
            return {"success": False, "error": f"Tracking error: {e!s}"}
