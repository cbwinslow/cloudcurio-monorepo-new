...
[00:05:00] Sarah: That's all for tonight, folks!"""
    ...
    def _search_episode_for_topics(
        self,
    ...
    def _get_context_segments(
        self, segments: list[dict[str, Any]], current_index: int, window_size: int
    ) -> list[dict[str, Any]]:
...
class JokeSearchTool(RobustTool):
    ...
    def __init__(self):
        super().__init__(
            name="search_jokes", description="Find jokes and humorous content about specific topics"
        )
    ...
    def _search_episode_for_jokes(
        ...
        for joke in jokes:
            ...
            topic_match = any(
                topic.lower() in [jt.lower() for jt in joke_topics] for topic in topics
            )
...
class ContextSearchTool(RobustTool):
        ...
        for segment in segments:
            ...
            if match_score > 0:
                result = {
                    ...
                    "match_details": {
                        "people_match": any(
                            p in segment_context.get("people", []) for p in parsed_context["people"]
                        ),
                        "location_match": any(
                            l in segment_context.get("location", "")
                            for l in parsed_context["locations"]
                        ),
                        "activity_match": any(
                            a in segment_context.get("activity", "")
                            for a in parsed_context["actions"]
                        ),
                    },
                }
    ...
    def _get_surrounding_context(
        self, segments: list[dict[str, Any]], current_segment: dict[str, Any]
    ) -> dict[str, Any]:
...
class TimestampSearchTool(RobustTool):
        ...
        return {
            ...
            "properties": {
                "time_specification": {
                    ...
                    "oneOf": [
                        {
                            "properties": {"exact_time": {"type": "string", "format": "time"}},
                            "required": ["exact_time"],
                        },
                        {
                            "properties": {
    ...
    def _search_episode_timestamps(
        self, episode_data: dict[str, Any], target_times: list[float], search_radius: float
    ) -> list[dict[str, Any]]:
        ...
        for target_time in target_times:
            ...
            for segment in segments:
                ...
                if (
                    abs(segment_start - target_time) <= search_radius
                    or abs(segment_end - target_time) <= search_radius
                    or (segment_start <= target_time <= segment_end)
                ):
...
class AdvancedSearchTool(RobustTool):
    ...
    def _execute_core(self, parameters: dict[str, Any], execution_id: str) -> Any:
        ...
        for episode_id, episode_data in comprehensive_index["episodes"].items():
            ...
            processed_episodes += 1
            episode_results = self._search_episode_advanced(episode_data, criteria, result_format)
    ...
    def _search_episode_advanced(
        self, episode_data: dict[str, Any], criteria: dict[str, Any], result_format: str
    ) -> list[dict[str, Any]]:
        ...
        for segment in segments:
            ...
            if "joke_topics" in criteria and segment.get("joke", False):
                ...
                joke_matches = sum(
                    1
                    for topic in criteria["joke_topics"]
            ...
            if "context" in criteria:
                segment_context = segment.get("context", {})
                context_text = (
                    f"{segment_context.get('location', '')} {segment_context.get('people', [])}"
                )
            ...
            if "locations" in criteria:
                ...
                location_matches = sum(
                    1 for location in criteria["locations"] if location.lower() == segment_location
                )
            ...
            if "people" in criteria:
                ...
                people_matches = sum(
                    1 for person in criteria["people"] if person.lower() in segment_people
                )
                if people_matches > 0:
...
class SearchIndexManagementTool(RobustTool):
    ...
    def _create_index(
        self, index_type: str, episode_ids: list[str], index_name: str, force_rebuild: bool
    ) -> dict[str, Any]:
    ...
    def _update_index(
        self, index_type: str, episode_ids: list[str], index_name: str
    ) -> dict[str, Any]:
    ...
    def _check_existing_index(self, index_name: str) -> dict[str, Any] | None:
...
if __name__ == "__main__":
    ...
    # Search for topics
    topic_results = agent.execute_tool(
        "search_by_topic", {"search_terms": ["AI", "future"], "max_results": 5}
    )
    print("Topic Search Results:", topic_results)

    # Search for jokes
    joke_results = agent.execute_tool(
        "search_jokes", {"topics": ["science", "technology"], "max_results": 3}
    )
    print("Joke Search Results:", joke_results)

    # Search by context
    context_results = agent.execute_tool(
        "search_by_context", {"context_query": "Jared live on stage", "max_results": 3}
    )
