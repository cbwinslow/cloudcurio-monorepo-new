...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/automation_agent.log"), logging.StreamHandler()],
)
...
class AutomationAgent:
    ...
    def check_alerts(self, metrics: dict) -> list[str]:
        ...
        for stream_id, stream_metrics in metrics.get("stream_quality", {}).items():
            if stream_metrics.get("latency", 0) > self.alert_thresholds.get(
                "stream_quality", {}
            ).get("latency", 1000):
                alerts.append(
                    f"High latency detected in stream {stream_id}: {stream_metrics.get('latency')}"
                )
            if stream_metrics.get("quality", "unknown") != self.alert_thresholds.get(
                "stream_quality", {}
            ).get("quality", "good"):
                alerts.append(
                    f"Poor quality detected in stream {stream_id}: {stream_metrics.get('quality')}"
                )

        # Check network traffic alerts
        for interface, network_metrics in metrics.get("network_traffic", {}).items():
            if network_metrics.get("rx_bytes", 0) > self.alert_thresholds.get(
                "network_traffic", {}
            ).get("rx_bytes", 1000000):
                alerts.append(
                    f"High incoming traffic on {interface}: {network_metrics.get('rx_bytes')}"
                )
            if network_metrics.get("tx_bytes", 0) > self.alert_thresholds.get(
                "network_traffic", {}
            ).get("tx_bytes", 1000000):
                alerts.append(
                    f"High outgoing traffic on {interface}: {network_metrics.get('tx_bytes')}"
                )

        # Check system health alerts
        for metric, system_metrics in metrics.get("system_health", {}).items():
            if system_metrics.get("value", 0) > self.alert_thresholds.get("system_health", {}).get(
                metric, 80
            ):
