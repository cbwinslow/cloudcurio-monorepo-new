...
from swarm_observability import A2AMessage, SwarmObservabilityManager
...
class ConversationLogger:
    ...
    def log_message(self, message: Message | A2AMessage) -> None:
        ...
        try:
            if isinstance(message, A2AMessage):
                ...
                metadata = (
                    json.dumps(message.content.get("_a2a_metadata", {}))
                    if "_a2a_metadata" in message.content
                    else None
                )
    ...
    def get_system_metrics(self, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                success_rate = (
                    (task_completion[0] / task_completion[2] * 100) if task_completion[2] > 0 else 0
                )
...
class PerformanceAnalyzer:
    ...
    def _analyze_message_patterns(self, metrics: dict[str, Any]) -> dict[str, Any]:
        ...
        expected_rate = self.baselines.get("message_rate", {}).get(
            "average", 10
        )  # messages per hour
    ...
    def _update_baselines(self, metrics: dict[str, Any]) -> None:
        ...
        self.baselines["message_rate"]["average"] = existing_avg + alpha * (
            current_rate - existing_avg
        )
        ...
        try:
            with sqlite3.connect(self.conversation_logger.db_path) as conn:
                conn.execute(
                    ...
                    WHERE alert_id = ?
                """,
                    (alert.resolved, alert.resolved_at, alert.resolution_note, alert.alert_id),
                )
        except Exception as e:
...
class SwarmMonitor:
    ...
    def _integrate_with_observability(self) -> None:
        ...
        def monitored_report_task(task_id: str, agent_name: str, success: bool, result: Any = None):
            ...
            self.conversation_logger.log_task(
                task_id, agent_name, "unknown", status, success, result
            )
    ...
    def _setup_default_alert_handlers(self) -> None:
        ...
        self.alert_manager.register_alert_handler(
            AlertType.PERFORMANCE_DEGRADATION, handle_performance_alert
        )
        ...
        self.alert_manager.register_alert_handler(
            AlertType.COMMUNICATION_ERROR, handle_communication_error
        )
    ...
    def get_monitoring_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_alerts": {"total": len(active_alerts), "by_severity": dict(alert_counts)},
