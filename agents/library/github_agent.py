...
class GitHubAgent(ToolBasedAgent):
    ...
    def get_github_issues(
        ...
        try:
            # Check cache first
            cache_key = f"{state}_{labels!s}_{assignee}_{limit}"
            current_time = time.time()

            if (
                cache_key in self.issues_cache
            ...
            cmd = [
                ...
                str(limit),
                "--json",
                "number,title,labels,assignees,state,createdAt,updatedAt,body",
            ]
            ...
            if result.returncode == 0:
                ...
                response = {"issues": issues, "metadata": metadata}
    ...
    def analyze_issue_content(self, issue_number: int) -> dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                f"{self.repo_owner}/{self.repo_name}",
                "--json",
                "number,title,labels,assignees,body,comments",
            ]
    ...
    def _analyze_issue_text(self, issue_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "requires_external_resources": any(
                word in body for word in ["api", "external", "third-party", "integration"]
            ),
        }
    ...
    def _extract_requirements(self, issue_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for line in lines:
            ...
            if is_task or any(
                ...
                for keyword in ["need", "should", "must", "implement", "create", "add", "fix"]
            ):
    ...
    def _assess_complexity(
        self, issue_data: dict[str, Any], requirements: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        complexity_factors = {
            ...
            "external_dependencies": any(
                req.get("requires_external_resources", False) for req in requirements
            ),
            "testing_required": any(
                "test" in req.get("description", "").lower() for req in requirements
            ),
            "documentation_required": any(
                "docs" in req.get("description", "").lower() for req in requirements
            ),
            "research_required": any(
                "research" in req.get("description", "").lower() for req in requirements
            ),
            "high_priority": any(
                label.lower() in ["urgent", "critical", "high-priority"] for label in labels
            ),
            "complex_keywords": any(
                word in body.lower()
                for word in ["complex", "difficult", "challenging", "architectural", "refactor"]
            ),
        }
    ...
    def _recommend_agents_for_issue(
        self, requirements: list[dict[str, Any]], labels: list[str]
    ) -> list[str]:
    ...
    def create_issue_summary_report(self, issues_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for issue in issues:
                ...
                category = "other"
                if any(label.lower() in ["bug", "fix", "error"] for label in labels) or any(
                    word in title for word in ["fix", "bug", "error"]
                ):
                    category = "bugs"
                elif any(label.lower() in ["feature", "enhancement"] for label in labels) or any(
                    word in title for word in ["add", "implement", "feature"]
                ):
                    category = "features"
                elif any(label.lower() in ["docs", "documentation"] for label in labels) or any(
                    word in body for word in ["readme", "documentation"]
                ):
            ...
            if priority_levels["urgent"] > 0:
                insights.append(
                    f"{priority_levels['urgent']} urgent issues require immediate attention"
                )
            ...
            report = {
                "summary": {
                    ...
                    "top_labels": sorted(
                        label_distribution.items(), key=lambda x: x[1], reverse=True
                    )[:10],
                    "assignee_workload": assignee_workload,
                },
                "insights": insights,
                "recommendations": self._generate_issue_recommendations(
                    categories, priority_levels, assignee_workload
                ),
    ...
    def _generate_issue_recommendations(
        self, categories: dict[str, list], priorities: dict[str, int], workloads: dict[str, int]
    ) -> list[str]:
        ...
        if priorities.get("urgent", 0) > 0:
            recommendations.append(
                "Focus on urgent issues first - allocate top talent to critical items"
            )
