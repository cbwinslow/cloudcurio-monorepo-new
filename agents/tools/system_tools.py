        ...
        try:
            if action == "list":
                ...
                if pid is None:
                    return {
                        "status": "error",
                        "error": "PID required for info action"
                    }
                ...
                if pid is None:
                    return {
                        "status": "error",
                        "error": "PID required for kill action"
                    }
...
class HealthCheckTool:
    ...
    def execute(self, thresholds: Optional[Dict[str, float]] = None, **kwargs: Any) -> Dict[str, Any]:
...
def system_info_tool(config: Optional[Dict[str, Any]] = None) -> SystemInfoTool:
    """Factory function for system info tool."""
    cfg = SystemConfig(**config) if config else SystemConfig()
    return SystemInfoTool(cfg)


def process_monitor_tool(config: Optional[Dict[str, Any]] = None) -> ProcessMonitorTool:
    """Factory function for process monitor tool."""
    cfg = SystemConfig(**config) if config else SystemConfig()
    return ProcessMonitorTool(cfg)


def resource_monitor_tool(config: Optional[Dict[str, Any]] = None) -> ResourceMonitorTool:
    """Factory function for resource monitor tool."""
    cfg = SystemConfig(**config) if config else SystemConfig()
    return ResourceMonitorTool(cfg)


def health_check_tool(config: Optional[Dict[str, Any]] = None) -> HealthCheckTool:
