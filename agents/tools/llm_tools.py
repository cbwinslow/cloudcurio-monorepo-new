...
class LLMConfig(BaseModel):
    ...
    base_url: str | None = Field(default="http://localhost:11434", description="Base URL")
...
class LLMCompletionTool:
    ...
    def _anthropic_completion(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
        ...
        return {"status": "error", "error": "Anthropic integration not yet implemented", "text": ""}
...
class LLMEmbeddingTool:
    ...
    def execute(self, text: str, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "embedding": []}
...
def llm_completion_tool(config: dict[str, Any] | None = None) -> LLMCompletionTool:
    """Factory function for LLM completion tool."""
    cfg = LLMConfig(**config) if config else LLMConfig()
    return LLMCompletionTool(cfg)


def llm_embedding_tool(config: dict[str, Any] | None = None) -> LLMEmbeddingTool:
