...
class WebConfig(BaseModel):
    ...
    user_agent: str = Field(
        default="Mozilla/5.0 (compatible; CloudCurio/1.0)",
...
class WebScraperTool:
    ...
    def execute(self, url: str, selector: Optional[str] = None, **kwargs: Any) -> Dict[str, Any]:
        """Scrape web page.
...
def web_scraper_tool(config: Optional[Dict[str, Any]] = None) -> WebScraperTool:
    """Factory function for web scraper tool."""
    cfg = WebConfig(**config) if config else WebConfig()
    return WebScraperTool(cfg)


def api_client_tool(config: Optional[Dict[str, Any]] = None) -> APIClientTool:
    """Factory function for API client tool."""
    cfg = WebConfig(**config) if config else WebConfig()
    return APIClientTool(cfg)


def search_engine_tool(config: Optional[Dict[str, Any]] = None) -> SearchEngineTool:
