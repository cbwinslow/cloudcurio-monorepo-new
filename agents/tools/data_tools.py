...
class DataTransformTool:
    ...
    def _aggregate_data(self, data: List[Dict], field: str, func: str) -> Union[int, float]:
...
def json_processor_tool(config: Optional[Dict[str, Any]] = None) -> JSONProcessorTool:
    """Factory function for JSON processor tool."""
    cfg = DataConfig(**config) if config else DataConfig()
    return JSONProcessorTool(cfg)


def data_transform_tool(config: Optional[Dict[str, Any]] = None) -> DataTransformTool:
    """Factory function for data transform tool."""
    cfg = DataConfig(**config) if config else DataConfig()
    return DataTransformTool(cfg)


def csv_processor_tool(config: Optional[Dict[str, Any]] = None) -> CSVProcessorTool:
