...
class FileReaderTool:
    ...
    def execute(self, filepath: str, encoding: str = "utf-8", **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if not path.exists():
                return {"status": "error", "error": f"File not found: {filepath}", "content": None}
            ...
            return {"status": "error", "error": str(e), "filepath": filepath, "content": None}
...
class FileWriterTool:
    ...
    def execute(
        self, filepath: str, content: str, mode: str = "w", encoding: str = "utf-8", **kwargs: Any
    ) -> dict[str, Any]:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if action == "list":
                ...
                return {"status": "success", "action": "create", "dirpath": str(path)}
                ...
                if path.is_dir():
                    ...
                    shutil.rmtree(path)
                    return {"status": "success", "action": "delete", "dirpath": str(path)}
                else:
                    return {
                        "status": "error",
            ...
            return {"status": "error", "error": str(e), "action": action, "dirpath": dirpath}
...
class FileSearchTool:
    ...
    def execute(
        self, query: str, search_type: str = "name", dirpath: str = ".", **kwargs: Any
    ) -> dict[str, Any]:
        """Search for files.
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "query": query, "matches": []}


def file_reader_tool(config: dict[str, Any] | None = None) -> FileReaderTool:
    """Factory function for file reader tool."""
    cfg = FileConfig(**config) if config else FileConfig()
    return FileReaderTool(cfg)


def file_writer_tool(config: dict[str, Any] | None = None) -> FileWriterTool:
    """Factory function for file writer tool."""
    cfg = FileConfig(**config) if config else FileConfig()
    return FileWriterTool(cfg)


def directory_tool(config: dict[str, Any] | None = None) -> DirectoryTool:
    """Factory function for directory tool."""
    cfg = FileConfig(**config) if config else FileConfig()
    return DirectoryTool(cfg)


def file_search_tool(config: dict[str, Any] | None = None) -> FileSearchTool:
