...
def query_llm(
    prompt: str, client=None, model=None, provider="openai", image_path: str | None = None
) -> str | None:
    """
...
def main():
    ...
    response = query_llm(
        args.prompt, client, model=args.model, provider=args.provider, image_path=args.image
    )
    if response:
        print(response)
    else:
