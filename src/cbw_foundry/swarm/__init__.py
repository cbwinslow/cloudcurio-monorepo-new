...
class Swarm:
    ...
    def __init__(self, name: str, agents: list[SwarmAgent], config: SwarmConfig | None = None):
        """Initialize swarm.
    ...
    def _execute_parallel(self, task: dict[str, Any]) -> Any:
        ...
        with ThreadPoolExecutor(max_workers=len(self.agents)) as executor:
            futures = {
                executor.submit(self._execute_agent, agent, task): agent for agent in self.agents
            }
    ...
    def _execute_democratic(self, task: dict[str, Any]) -> Any:
        ...
        for agent in self.agents:
            ...
            proposals[agent.name] = {"result": result, "confidence": agent.confidence}
            self._results[agent.name] = result

        # Democratic voting on best result
        best_proposal = max(proposals.items(), key=lambda x: x[1]["confidence"])
