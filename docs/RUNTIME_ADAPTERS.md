    ...
    def run(self, spec_path: str, user_input: str) -> RunResult:
        ...
        if tool:
            ...
            if not mod_fs.exists():
                return RunResult(
                    output={"error": f"tool module not found: {mod_fs}"}, runtime=self.name
                )
            ...
            if fn is None:
                return RunResult(
                    output={"error": f"tool func not found: {func}"}, runtime=self.name
                )
...
class CrewAIRuntime(AgentRuntime):
    ...
    def run(self, spec_path: str, user_input: str) -> RunResult:
        ...
        # Create task
        task = Task(description=user_input, agent=agent, expected_output="Processed result")
        ...
        try:
            ...
            return RunResult(output=None, runtime=self.name, error=str(e))
...
class PydanticAIRuntime(AgentRuntime):
    ...
    def run(self, spec_path: str, user_input: str) -> RunResult:
        ...
        agent = PydanticAgent(model=model_name, system_prompt=system_prompt)
        ...
        try:
            ...
            return RunResult(
                ...
                runtime=self.name,
                metadata={"agent": agent_spec.metadata.name, "model": model_name},
            )
        except Exception as e:
            return RunResult(output=None, runtime=self.name, error=str(e))
...
class LangChainRuntime(AgentRuntime):
    ...
    def run(self, spec_path: str, user_input: str) -> RunResult:
        ...
        try:
            ...
            return RunResult(
                output=result, runtime=self.name, metadata={"agent": agent_spec.metadata.name}
            )
        except Exception as e:
            return RunResult(output=None, runtime=self.name, error=str(e))
...
class CustomRuntime(AgentRuntime):
    ...
    def run(self, spec_path: str, user_input: str) -> RunResult:
        ...
        try:
            ...
            # 4. Format result
            return RunResult(
                output=output, runtime=self.name, metadata=self._get_metadata(agent_spec)
            )
        except Exception as e:
            return RunResult(output=None, runtime=self.name, error=str(e))
    ...
    def _get_metadata(self, spec: AgentSpec) -> dict:
        ...
        return {"agent_name": spec.metadata.name, "agent_version": spec.metadata.version}
```
...
def test_custom_runtime_execution():
    ...
    result = runtime.run(spec_path="agents/specs/test_agent.agent.yaml", user_input="test input")
...
def test_custom_runtime_error_handling():
    ...
    result = runtime.run(spec_path="nonexistent.yaml", user_input="test")
...
class TestCrewAIRuntime:
    ...
    def test_execution(self, mock_kickoff):
        ...
        result = self.runtime.run("agents/specs/test.agent.yaml", "test input")
    ...
    def test_error_handling(self):
        """Test error handling."""
        result = self.runtime.run("nonexistent.yaml", "test")
...
class TestRuntimeIntegration:
    ...
    def test_local_runtime_integration(self):
        ...
        result = runtime.run("agents/specs/examples/hello_world.agent.yaml", "Hello, world!")
    ...
    def test_crewai_runtime_integration(self):
        ...
        result = runtime.run("agents/specs/examples/hello_world.agent.yaml", "Test task")
